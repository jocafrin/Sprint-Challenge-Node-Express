...
router.post("/", checkProjectId, (req, res) => {
  ...
  if (!body.description || !body.notes) {
    res
      .status(400)
      .json({
        error: "Please provide a description and notes for this action."
      });
    ...
    res
      ...
      .json({
        error:
          "Please provide a description that is less than 128 characters long."
      });
  } else
    actions
      .insert(body)
    ...
    actions
      ...
      .then(action => {
        if (action === null) {
          ...
          res
            .status(200)
            .json({ message: "The action was updated successfully." });
        }
      })
      .catch(err => {
        console.error(err);
